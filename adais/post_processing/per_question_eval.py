...
import pandas as pd
import tqdm
from adais.post_processing import aggregators
...
def run_eval_for_num_traces(
    ...
    def run_eval(metric_name: str, eval_func: Callable[[Any], Any], samples, label):
...
def _is_one_correct(answer1, answer2, golden_label):
    ...
    no_nans = not is_none_or_empty_string(answer1) and not is_none_or_empty_string(
        answer2
    )
...
def _calc_wqd_eval_for_row(
    ...
    samples = [rnd.sample(traces, 2) for _ in range(num_bootstrap) if len(traces) > 1]
